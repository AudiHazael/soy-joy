...
function ready() {
  ...
  const quantityInputs = document.getElementsByClassName("class_quantity");
  for (let i = 0; i < quantityInputs.length; i++) {
    const input = quantityInputs[i];
    input.addEventListener("change", quantityChanged);
  }

  const addCartButtons = document.getElementsByClassName("add_cart");
...
function updateTotal() {
  ...
  if (document.getElementsByClassName("total_price_two")[0]) {
    document.getElementsByClassName("total_price_two")[0].innerHTML =
      "$" + total;
  }
}
